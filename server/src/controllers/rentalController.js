...
const normalizeRentalCarBody = (body) => {
  ...
  if (body.registrationNumber) body.registrationNumber = String(body.registrationNumber).trim().toUpperCase();
  if (body.bikeNumber) body.bikeNumber = String(body.bikeNumber).trim();
...
const deleteRentalCar = asyncHandler(async (req, res) => {
  ...
  if (!car) { res.status(404); throw new Error('Rental bike not found'); }
  res.json({ success: true, message: 'Rental bike deleted' });
});
...
const createRentalBooking = asyncHandler(async (req, res) => {
  ...
  if (unit === 'hour' && (!car.pricePerHour || car.pricePerHour <= 0)) {
    res.status(400);
    throw new Error('Hourly rental is not available for this bike');
  }
  ...
  // Conflict check — only paid/active bookings block. Stale 'requested' bookings older
  // than 15 minutes are ignored. Bookings created by the SAME user are also ignored.
  ...
  if (candidates.length) {
    ...
    if (realConflict) {
      res.status(400);
      throw new Error('Bike is already booked for the selected time window');
    }
  }
...
const getMyRentalBookings = asyncHandler(async (req, res) => {
  const bookings = await RentalBooking.find({ user: req.user._id })
    .populate('rentalCar', 'title brand model year images pricePerDay pricePerHour rentalUnits registrationNumber bikeNumber rcNumber chassisNumber engineNumber insuranceValidTill pucValidTill location dropLocation fuelType transmission seats mileage securityDepositCompulsory')
    .sort({ createdAt: -1 });
...
const getActiveLocations = asyncHandler(async (req, res) => {
  const bookings = await RentalBooking.find({ status: 'active' })
    .select('currentLocation carSnapshot user totalDays pickupDate returnDate')
    .populate('user', 'name phone email');
  res.json({ success: true, bookings });
});

// @desc  Create Razorpay order for pending balance (user pays the remaining balance online)
// @route POST /api/rentals/bookings/pay-balance/:id
const createRentalBalanceOrder = asyncHandler(async (req, res) => {
  ...
  try {
    const options = {
      ...
      currency: 'INR',
      receipt: `balance_${booking._id}`.slice(0, 40),
    };
    const order = await razorpay.orders.create(options);
    booking.payment.razorpayOrderId = order.id;
    await booking.save();
...
// @route POST /api/rentals/bookings/verify-balance/:id
